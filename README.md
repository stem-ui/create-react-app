...
This repo is a fork of create-react-app so that we can do some deeper customization not supported by the public API, without having to eject the project and lose community-supported dependency managment.

The original project is tracked as the "upstream" remote. `git fetch upstream && git rebase upstream/master` to update and then re-publish the project.
